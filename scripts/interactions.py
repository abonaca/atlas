...
def scaling(seed=98, f=2):
    ...
    labels = ['M,T,V', '{0:.1f}M,T/{0:.1f},V'.format(f), '{0:.1f}M,T,{0:.1f}V'.format(f),'M,{0:.1f}T,{0:.1f}V'.format(f)]
    
    plt.close()
    fig, ax = plt.subplots(1,2,figsize=(10,5))
    
    for e, stream in enumerate(streams):
        color = mpl.cm.Blues(e/4+0.1)
        ms = 10 - 2*e
...
def scaling_norm(seed=473):
    ...
    plt.plot(f3_array**2, sigma_x_vt, color=lblue, lw=2)
    ...
    plt.plot(f3_array**2, sigma_v_vt, color=lblue, lw=2)
